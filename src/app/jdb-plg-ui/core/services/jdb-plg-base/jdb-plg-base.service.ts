...
export class JdbPlgBaseService {
    ...
    post(apiName, dataObj, isIntercept = true) {
        const jdbDhTraceId = new Date().getTime() + '-' + parseInt(`${Math.random() * (100000 + 1) + 1}`, 10);
        let loginToken = localStorage.getItem("cxLoginToken");
        let loginWay = localStorage.getItem("cxLoginWay");
        let orgUid = localStorage.getItem("orgUid");
        let loginObj: any = {};
        let data: any = {};
        ...
        return this.http.request(reqUrl, requestoptions)
            ...
            .filter((res: any) => {
                ...
                if (res.error && res.error.returnCode * 1 === 105 && currentRoute != 'login') {
                    this.route.navigate(['login']);
                    return false;
                }
